...
*/
{
  ...
  digitalWrite(6,LOW);//turns UTI off
...
void loop() 
{
  ...
  if (client) 
      ...
      while (client.connected()) 
	{
      ...
      if (strncmp(ret_string,"GetCap\n", 6) == 0)//string comapre from client 
	{
      ...
      else if (strncmp(ret_string,"SetMode High\n", 12) == 0)
	{
	  mode_select=High;//string compare mode selector 
	  set_UTI_mode();//refers back to mode slector function 
	}

      else if (strncmp(ret_string,"SetMode Low\n", 11) == 0)
	{
	  mode_select=Low;
	  set_UTI_mode();
	}
    }
  client.stop();// stops client connection 
}
